...
Verified with 'pingall': only the h1↔h3 pair failed (33% dropped, 4/6 received), while h1↔h2 and h2↔h3 continued working normally. Confirmed the rule was active and dropping packets (n_packets=11) using 'dump-flows s1'. <br> 
...
Found the '_packet_in_handler' method (decorated with '@set_ev_cls(ofp_event.EventOFPPacketIn, MAIN_DISPATCHER)') and the 'add_flow()' call inside it. <br> 
